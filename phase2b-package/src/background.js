...
import { getTokenBalances, buildTokenTransfer } from "./lib/tokens.js";
import { fetchTokenMetadata } from "./lib/metadata.js";
...
async function handleGetTokenBalances() {
  ...
  const connection = getConnection(network || "devnet");
  try {
    const tokens = await getTokenBalances(connection, keypair.publicKey);

    // Fetch metadata for each token in parallel. Any individual failure
    // just leaves that token without a name/logo — never blocks the rest.
    const enriched = await Promise.all(
      tokens.map(async (t) => {
        const meta = await fetchTokenMetadata(connection, t.mint).catch(() => null);
        return {
          ...t,
          name: meta?.name || null,
    ...
    return { ok: true, tokens: enriched };
  } catch (err) {
    return { ok: false, error: err.message || "Couldn't fetch token balances." };
  }
}
...
async function handleSendTokenTransfer({ to, mint, rawAmount }) {
  const keypair = await getActiveKeypair();
  if (!keypair) return { ok: false, error: "Wallet is locked." };
  ...
  try {
    ...
    return { ok: false, error: err.message || "Token transfer failed." };
  }
}
...
const handlers = {
  ...
  SEND_TOKEN_TRANSFER: handleSendTokenTransfer,
